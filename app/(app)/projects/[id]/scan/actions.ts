...
export async function logFileRecord(input: {
  ...
  const accessError = await requireStageAccess(supabase, user.id, parsed.data.projectId, "scan");
...
export async function advanceStage(projectId: string, _newStatus: ProjectStatus): Promise<{ error?: string }> {
  ...
  if (proj.status !== "scan") return { error: "Project is not at the Scan stage." };

  const accessError = await requireStageAccess(supabase, user.id, projectId, "scan");
  if (accessError) return { error: accessError };
