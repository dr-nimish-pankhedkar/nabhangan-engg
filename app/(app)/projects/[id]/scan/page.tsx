...
export default async function ScanPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [submissionRes, prevFilesRes, projectRes, profileRes] = await Promise.all([
    ...
    db.from("project_files").select("id, file_name, file_path, file_type, stage, uploaded_at").eq("project_id", id).in("stage", ["survey", "rate_verification", "drafting", "print"]).order("uploaded_at"),
