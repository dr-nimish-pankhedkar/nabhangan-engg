...
export default function ScanClient({ projectId, userId }: { projectId: string; userId: string }) {
  ...
  async function handleFileUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const url = await uploadProjectFile(projectId, "scan", file);
      setUploadProgress(80);
      await logFileRecord({ projectId, userId, stage: "scan", filePath: url, fileName: file.name, fileType: file.type });
  ...
  async function onTimeSubmit(data: any) {
    const result = await logTime({ projectId, userId, stage: "scan", hours_spent: parseFloat(data.hours_spent), notes: data.notes || null });
