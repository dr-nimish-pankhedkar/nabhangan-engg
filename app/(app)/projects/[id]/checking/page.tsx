...
const THIS_STAGE_IDX = 4; // checking
...
export default async function CheckingPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [submissionRes, prevFilesRes, projectRes, profileRes] = await Promise.all([
    ...
    db.from("project_files").select("id, file_name, file_path, file_type, stage, uploaded_at").eq("project_id", id).in("stage", ["survey", "rate_verification", "drafting"]).order("uploaded_at"),
