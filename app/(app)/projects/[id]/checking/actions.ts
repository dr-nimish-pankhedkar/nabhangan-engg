...
export async function logFileRecord(input: {
  ...
  const accessError = await requireStageAccess(supabase, user.id, parsed.data.projectId, "checking");
...
export async function advanceStage(projectId: string, _newStatus: ProjectStatus): Promise<{ error?: string }> {
  ...
  if (proj.status !== "checking") return { error: "Project is not at the Checking stage." };

  const accessError = await requireStageAccess(supabase, user.id, projectId, "checking");
  if (accessError) return { error: accessError };
  ...
  const { error } = await supabase.from("projects").update({ status: "print" }).eq("id", projectId);
  if (error) return { error: error.message };
  return {};
}
