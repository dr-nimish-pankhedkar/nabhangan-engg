...
function Field({ label, value }: { label: string; value: string }) {
  return (
    <div className="py-0.5 px-2 border-b border-slate-100 last:border-0">
      <div className="text-[9px] uppercase tracking-wide text-slate-400">{label}</div>
      <div className="text-xs text-slate-800 font-medium min-h-[14px]">
...
function SectionHeading({ n, children }: { n: number; children: React.ReactNode }) {
  return (
    <div className="flex items-center gap-1.5 mt-2 mb-0 first:mt-0">
      <div className="w-4 h-4 rounded-full bg-[#1e3a5f] text-white text-[9px] font-bold flex items-center justify-center shrink-0">
        {n}
      </div>
      <h2 className="text-[10px] font-bold uppercase tracking-widest text-[#1e3a5f]">{children}</h2>
  ...
  return (
    <>
      ...
      >
        {/* Document Header */}
        <div
          className="flex items-start justify-between gap-4 pb-2 mb-2"
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <h1 className="text-xl font-extrabold text-[#1e3a5f] leading-tight">
                Site Survey Report
              </h1>
        ...
        {r.remark && (
          <>
            ...
            <div className="border border-slate-200 rounded-md px-2 py-1.5 text-xs text-slate-700 whitespace-pre-wrap leading-snug">
              {r.remark}
            </div>
          </>
        )}
        ...
        {validPhotos.length > 0 && (
          <>
            ...
            >
              {validPhotos.slice(0, 2).map((photo) => (
                <div
                  key={photo.id}
