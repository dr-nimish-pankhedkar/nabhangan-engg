...
export async function advanceStage(projectId: string, newStatus: ProjectStatus): Promise<{ error?: string }> {
  ...
  if (proj.status !== "fees_received") return { error: "Project is not yet at the Fees Received stage." };
  ...
  const { error } = await admin.from("projects").update({ status: newStatus }).eq("id", projectId);
  if (error) return { error: error.message };
  return {};
}
